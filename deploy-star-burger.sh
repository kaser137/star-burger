...
curl -H "X-Rollbar-Access-Token: $ROLLBAR_TOKEN" -H "Content-Type: application/json" -X POST 'https://api.rollbar.com/api/1/deploy' -d '{"environment": "production", "revision": "'"$COMMIT"'", "rollbar_name": "buzhyn", "local_username": "'"$USER"'", "comment": "'"$COMMENT"'", "status": "succeeded"}'
